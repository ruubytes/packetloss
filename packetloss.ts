interface ResultNode {
  ...
  const main = () => {
    ...
    const measurePacketloss = async () => {
      ...
      const endTime: number | null = timeout ? startTime + (timeout * 1000) : null;
      try {
        while (isRunning) {
          if (endTime && Date.now() >= endTime) break;
          const pingTime = Date.now();
          const website: Website = websites[Math.floor(Math.random() * websites.length)];
          const packetStatus: boolean = await ping(website);
          recordPacket(packetStatus);
          await writeStats();
          await sleep(Math.max(0, 1000 - (Date.now() - pingTime)));
        }
      } finally {
